...
public class VoxelRenderer : MonoBehaviour {
	...
	// }
	public Vector3 RenderVoxelPos(Transform bone,Vector3 bindPose){
		return VoxelRenderManager.Fixed(bone.transform.position-( bone.right*bindPose.x+bone.forward*bindPose.z+bone.up*bindPose.y),GridSize);
	}
	 private void Update() {
		if(boneRoot!=null&&bindPose!=null){
			for (int i = 0; i < bindPose.Length; i++)
			{
				VoxelRenererPool.Instance.SetVoxel(RenderVoxelPos(bones[bindIndex[i]],bindPose[i]),voxelData.voxels[i].colorIndex);
			}
		}
